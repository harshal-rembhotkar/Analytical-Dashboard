...
export default function NotFound() {
  ...
  return (
    <div className="rounded-lg border border-gray-200 dark:border-gray-700 bg-[var(--card)] p-8 text-center">
      ...
      {error?.statusText && <p className="text-xs text-gray-400 mb-4">{String(error.statusText)}</p>}
      <Link className="px-4 py-2 rounded-md bg-blue-600 text-white" to="/">Go to Dashboard</Link>
    </div>
  )
}
