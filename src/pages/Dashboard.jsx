...
import { avgDatapoint, avgMTS, computeSummary, padHistogramBuckets, sumDatapoint, sumMTS } from '../util'
...
export default function Dashboard() {
  ...
  const { data: histogram, isLoading: loadingHistogram, isError: errorHistogram } = useQuery({
    ...
    queryFn: async () => {
      const data = await getMetricsHistogram(collection_name, { start_time, page_size: interval_count * 2, interval_minutes })
      data.buckets = padHistogramBuckets(start_time, interval_minutes, interval_count * 2, data.buckets);
      return data;
    },
  })
  ...
  const { current: currentAvgMTS, prev: prevAvgMTS, change: percentChangeMTS } = useMemo(() => {
    return computeSummary(histogram, avgMTS);
  }, [histogram])

  const { current: currentTotalMTS, prev: prevTotalMTS, change: totalChangeMTS } = useMemo(() => {
    return computeSummary(histogram, sumMTS);
  }, [histogram])

  const { current: currentAvgDatapoints, prev: prevAvgDatapoints, change: percentChangeDatapoints } = useMemo(() => {
    return computeSummary(histogram, avgDatapoint)
  }, [histogram])

  const { current: currentTotalDatapoints, prev: prevTotalDatapoints, change: totalChangeDatapoints } = useMemo(() => {
    return computeSummary(histogram, sumDatapoint)
  }, [histogram])

  const { current: currentPointsPerMTS, prev: prevPointsPerMTS, change: changePointsPerMTS } = useMemo(() => {
  ...
  const { current: currentAvgPointsPerMTS, prev: prevAvgPointsPerMTS, change: changeAvgPointsPerMTS } = useMemo(() => {
    return computeSummary(histogram, (buckets) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <MetricCard title="Total MTS" value={currentTotalMTS.toLocaleString()} delta={totalChangeMTS} subtitle={`Prev: ${prevTotalMTS.toLocaleString()}`} />
        <MetricCard title="Total Datapoints" value={currentTotalDatapoints.toLocaleString()} delta={totalChangeDatapoints} subtitle={`Prev: ${prevTotalDatapoints.toLocaleString()}`} />
        <MetricCard title="Datapoints/MTS" value={currentPointsPerMTS.toLocaleString()} delta={changePointsPerMTS} subtitle={`Prev: ${prevPointsPerMTS.toLocaleString()}`} />
        <MetricCard title="Avg MTS" value={currentAvgMTS.toLocaleString()} delta={percentChangeMTS} subtitle={`Prev: ${prevAvgMTS.toLocaleString()}`} />
        <MetricCard title="Avg Datapoints" value={currentAvgDatapoints.toLocaleString()} delta={percentChangeDatapoints} subtitle={`Prev: ${prevAvgDatapoints.toLocaleString()}`} />
        <MetricCard title="Avg Datapoints/MTS" value={currentAvgPointsPerMTS.toLocaleString()} delta={changeAvgPointsPerMTS} subtitle={`Prev: ${prevAvgPointsPerMTS.toLocaleString()}`} />
      </div>
