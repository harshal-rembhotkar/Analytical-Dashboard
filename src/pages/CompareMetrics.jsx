...
export default function MetricDetail() {
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: async () => {
      const data = await Promise.all(
        metrics.map((metric) =>
          getMetricDetail(collection_name, metric, {
            start_time,
            page_size: 12,
            interval_minutes,
          }),
  ...
  const chartData = useMemo(() => {
    ...
    const chartData = data[0].buckets.map((bucket) => ({
      time: bucket.timestamp.toLocaleTimeString([], {
        hour: "2-digit",
        minute: "2-digit",
      }),
  ...
  return (
    <div className="space-y-4">
      ...
      {isError && (
        <div className="text-sm text-red-600">Failed to load metric.</div>
      )}
